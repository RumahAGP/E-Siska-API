...
export const inputAbsensiValidation = [
  ...
  body("data").isArray({ min: 1 }).withMessage("Data absensi harus berupa array dan tidak boleh kosong"),
  body("data.*.siswaId").isUUID().withMessage("Format siswaId tidak valid"),
  body("data.*.status")
    .isIn(Object.values(AbsensiStatus))
    .withMessage(`Status harus salah satu dari: ${Object.values(AbsensiStatus).join(", ")}`),
  validationHandler,
];